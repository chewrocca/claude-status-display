# Claude Code status display (Waveshare ESP32-C6-LCD-1.47)

A desk gadget that shows what Claude Code is doing: context window used, 5-hour and
weekly rate-limit burn, whether Claude is working or waiting on you, and whether the
Claude API has an incident. The board's RGB LED mirrors the state so you can read it
...
| `host/com.chewrocca.claude-esp32-status.plist` | launchd agent that keeps the daemon running. Installed to `~/Library/LaunchAgents/`. |
...
## Wi-Fi and Bluetooth (firmware 4.x)
...
launchctl bootout gui/$(id -u)/com.chewrocca.claude-esp32-status   # free the serial port
...
arduino-cli upload  --fqbn esp32:esp32:esp32c6:CDCOnBoot=cdc,PartitionScheme=no_ota --port /dev/cu.usbmodem1101 --input-dir firmware/build firmware/claude_status
launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.chewrocca.claude-esp32-status.plist
```
...
```sh
launchctl bootout gui/$(id -u)/com.chewrocca.claude-esp32-status
