...
| `host/claude_status_daemon.py` | uv script. Reads state files and session transcripts, polls status.claude.com, pushes one JSON line per change over USB serial or HTTP. |
| `host/esp32-status-hook.sh` | Claude Code hook. Records per-session working / done / needs_input, plus the cwd and transcript path. Installed to `~/.claude/hooks/`. |
| `host/install.sh` | Everything outside the repo: hook, status line patch, hook registration, launchd agent. Also served by the board. |
| `host/com.claude-status.display.plist` | launchd agent that keeps the daemon running. Installed to `~/Library/LaunchAgents/`. |
| `host/gen_payload.py` | Embeds `install.sh`, the hook and the daemon into the firmware, so the board can install a machine on its own. Re-run when any of those change. |
...
Hooks write `~/.claude/esp32-status/attention/<session_id>.json`. The daemon merges the session
that owns the headline, the attention files, and the status page, then writes one JSON line to
`/dev/cu.usbmodem*` on change or every 3 s, falling back to `POST /status` when there is no
cable. Those two directories are the only state it keeps: the window sizes and the weekly curve
live on the board.
